...
for i in range(0,len(leps)):
  for j in range(0, len(nsdl)):
    for k in range (0, len(rldl)):
      jobs.append (Job('0a%d_%d_%d' % (i,j,k), 'solfec examples/parallel-scaling/array-of-cubes.py -subd reldelta -outi 0.3 -lmxi 1000 -leps %g -nsdl %g -rldl %s -prfx A' % (leps[i], nsdl[j], rldl[k])))

for i in range(0,len(lmxi)):
  for j in range(0, len(nsdl)):
    for k in range (0, len(rldl)):
      jobs.append (Job('0b%d_%d_%d' % (i,j,k), 'solfec examples/parallel-scaling/array-of-cubes.py -subd reldelta -outi 0.3 -lmxi %d -leps 1E-3 -nsdl %g -rldl %s -prfx B' % (lmxi[i], nsdl[j], rldl[k])))

for i in range(0,len(lepsC)):
  for j in range(0, len(nsdl)):
    for k in range (0, len(rldl)):
      jobs.append (Job('0c%d_%d_%d' % (i,j,k), 'solfec examples/parallel-scaling/array-of-cubes.py -subd reldelta -outi 0.3 -lmxi %d -leps %g -nsdl %g -rldl %s -prfx C' % (lmxiC[i], lepsC[i], nsdl[j], rldl[k])))
