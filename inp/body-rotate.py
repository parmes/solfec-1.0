...
def scene_run (step, stop, kinem, formul, damp):
  ...
  solfec = SOLFEC ('DYNAMIC', step, 'out/body-rotate/' + kinem + '_' + str(step) + '_' + str (stop) + '_' + formul + '_' + str (damp))
...
if not VIEWER() and pairs [0][0].mode == 'READ':
  ...
  try:
    ...
    plt.savefig ('out/body-rotate/fb_dd_step005_eta0.eps')
  except ImportError:
    pass # no reaction
  ...
  try:
    ...
    plt.savefig ('out/body-rotate/fb_dy_step005_eta0.eps')
  except ImportError:
    pass # no reaction
  ...
  try:
    ...
    plt.ylabel ('Energy [J]')
    plt.legend(loc = 'upper right')
    plt.savefig ('out/body-rotate/fb_ene_step005_eta0.eps')
  except ImportError:
    pass # no reaction
  ...
  try:
    ...
    plt.ylabel ('Inner diamter [m]')
    plt.legend(loc = 'upper left')
    plt.savefig ('out/body-rotate/fb_dd_step001_eta1.eps')
  except ImportError:
    pass # no reaction
