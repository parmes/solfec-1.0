...
if argv == None:
  print '------------------------------------------------------'
  print 'SYNOPSIS: solfec 81postp.py path/to/81array.thv'
  print '------------------------------------------------------'
  print 'You must first run analysis for 81array.py and then'
  print 'run it again in read mode to extract the  *.thv file'
  print '------------------------------------------------------'
  sys.exit ()

success = 0
for path in argv:
  if path.endswith ('.thv'):
    try:
      f = open (path, 'r')
      success = 1
      f.close ()
...
with open(outpath + '_VX_UNVERIFIED.txt', 'w') as fout:
  ...
  for i, p in enumerate(plotorder):
    ...
    newax.plot(winfreqs, nor_pkVs[p], '-', label=p)
  ...
  plotpath = outpath + '_VX_UNVERIFIED.png'
  fig1.savefig(plotpath)
  
  # record we've successfully saved this:
  fout.write('Saved plot %s' % plotpath)
