...
int *cs_amd (int order, const MX *A)  /* order 0:natural, 1:Chol, 2:LU, 3:QR */
{
    MX *C, *A2, *AT ;
...
csn *cs_chol (const MX *A, const css *S)
{
    ...
    C = pinv ? cs_symperm (A, pinv, 1) : ((MX *) A) ;
...
static void init_ata (MX *AT, const int *post, int *w, int **head, int **next)
{
...
static int cs_bfs (const MX *A, int n, int *wi, int *wj, int *queue,
    const int *imatch, const int *jmatch, int mark)
{
    ...
    C = (mark == 1) ? ((MX *) A) : cs_transpose (A, 0) ;
...
static void cs_augment (int k, const MX *A, int *jmatch, int *cheap, int *w,
...
int *cs_maxtrans (const MX *A, int seed)  /*[jmatch [0..m-1]; imatch [0..n-1]]*/
{
    ...
    C = (m2 < n2) ? cs_transpose (A,0) : ((MX *) A) ; /* transpose if needed */
...
csd *cs_scc (MX *A)     /* matrix A temporarily modified, then restored */
{
...
int cs_spsolve (MX *G, const MX *B, int k, int *xi, double *x, const int *pinv,
    int lo)
{
...
static int cs_vcount (const MX *A, css *S)
{
...
css *cs_sqr (int order, const MX *A, int qr)
{
    ...
    {
        MX *C = order ? cs_permute (A, NULL, S->q, 0) : ((MX *) A) ;
