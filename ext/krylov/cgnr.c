   ...
   )
{
   ...
   cgnr_functions = (hypre_CGNRFunctions *)
      CAlloc( 1, sizeof(hypre_CGNRFunctions) );
...
hypre_CGNRCreate( hypre_CGNRFunctions *cgnr_functions )
{
   ...
   cgnr_data = hypre_CTAllocF( hypre_CGNRData, 1, cgnr_functions);
...
hypre_CGNRDestroy( void *cgnr_vdata )
{
   ...
   if (cgnr_data)
   {
      ...
      {
         hypre_TFreeF(cgnr_data -> norms, cgnr_functions);
      }
      ...
      hypre_TFreeF(cgnr_data, cgnr_functions);
      hypre_TFreeF(cgnr_functions, cgnr_functions);
   }
                ...
                void *x         )
{
   ...
   if ((cgnr_data -> logging) > 0)
   {
      (cgnr_data -> norms)     = hypre_CTAllocF(double, max_iter + 1, cgnr_functions);
