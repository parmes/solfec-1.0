   ...
   )
{
   ...
   bicgstab_functions = (hypre_BiCGSTABFunctions *)
      CAlloc( 1, sizeof(hypre_BiCGSTABFunctions) );

   bicgstab_functions->CAlloc = CAlloc;
   bicgstab_functions->Free = Free;
...
hypre_BiCGSTABCreate( hypre_BiCGSTABFunctions * bicgstab_functions )
{
   ...
   bicgstab_data = hypre_CTAllocF( hypre_BiCGSTABData, 1, bicgstab_functions);
...
hypre_BiCGSTABDestroy( void *bicgstab_vdata )
{
   ...
   if (bicgstab_data)
   {
      if ( (bicgstab_data -> norms) != NULL )
            hypre_TFreeF(bicgstab_data ->norms, bicgstab_functions);
      ...
      hypre_TFreeF(bicgstab_data, bicgstab_functions);
      hypre_TFreeF(bicgstab_functions, bicgstab_functions);
   }
                  ...
                  void *x         )
{
   ...
   {
      if ((bicgstab_data -> norms) == NULL)
         (bicgstab_data -> norms) = hypre_CTAllocF(double, max_iter + 1, bicgstab_functions);
   }
